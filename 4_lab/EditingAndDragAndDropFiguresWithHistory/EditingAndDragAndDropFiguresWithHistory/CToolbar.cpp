	...
	m_state(state) 
{
	...
	//кнопка Undo
	auto triangleUndo = new ConvexShape();
	triangleUndo->setPointCount(3);
	triangleUndo->setPoint(0, Vector2f(462, 30));
	triangleUndo->setPoint(1, Vector2f(490, 15));
	triangleUndo->setPoint(2, Vector2f(490, 45));
	triangleUndo->setOutlineColor(Color::Black);
	triangleUndo->setOutlineThickness(2);
	m_buttons.push_back(new CButton(Vector2f(455, 10), Vector2f(40, 40), Color::White, new CUndoCommand(this), "", 0, triangleUndo));
